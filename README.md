...
### Checklist de merge limpio

Antes de fusionar el pull request, verificar que la rama final incluya estos archivos:
...
El workflow ejecuta el job `build` en `pull_request` contra `main` o `master` para validar que los archivos reales de despliegue existan antes del merge; el job `deploy` solo publica en push o ejecución manual.
